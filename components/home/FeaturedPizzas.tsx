				...
				{loading ? (
					<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6'>
						{[...Array(4)].map((_, i) => (
							<div
								key={i}
								className='bg-white rounded-3xl overflow-hidden shadow-md border border-border h-[420px] animate-pulse'
							>
								...
								>
									<Link href={`/menu`}>
										...
										<div className='relative h-52 overflow-hidden'>
											...
											{/* Tag */}
											<span
												className={`absolute top-3 left-3 text-xs font-bold px-2.5 py-1 rounded-full ${categoryColors[catName] || 'bg-orange-100 text-orange-700'}`}
											>
											...
											<div className='flex items-start justify-between mb-1.5'>
												...
												<div className='flex items-center gap-0.5 shrink-0 ml-2'>
													...
													<span className='text-xs font-bold text-dark'>
														{pizza.averageRating || 5.0}
													</span>
												</div>
											</div>
											<p className='text-muted-fg text-sm leading-relaxed line-clamp-2 mb-4'>
												{lang === 'bn' && pizza.description_bn
													? pizza.description_bn
													: pizza.description}
											</p>
